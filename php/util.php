...
function checkUrl($url, $returnError = false) {
	...
	if ($httpCode >= 200 && $httpCode < 300) {
		...
		write_log("Connection failed, error code is $httpCode, message is '$errMsg'?");
...
function decodeResult($result, $contentType = false, $log = true) {
	...
	if (is_string($result) && $contentType) {
		...
		if ($log) write_log("Unable to decode data with specified content type of $contentType");
	}
...
function getCert() {
	...
	if (file_exists($file)) {
		...
		write_log("Fetching updated SSL Certificate.", "INFO");
...
function getLocale() {
	...
	if (!$locale) {
		write_log("Failed to select a language automatically, defaulting to English.", "ERROR");
		$locale = "en";
	}
...
function headerHtml() {
	...
	foreach (plexHeaders() as $key => $value) {
		if (is_string($value)) {
			...
			write_log("Value is an array: " . json_encode($value), "WARN");
		}
	}
	$string .= "></div>";
	return $string;
	...
	foreach ($items as $final) {
		...
		switch (true) {
			...
			case ($singleName && !$singleType && !$singleTypeItem && !$singleNameItem): //1000, "Play Toy Story"
				$itemString = "$name ($parent)";
				if ($type === 'track') $itemString = "$name (track by $parent)";
...
function listLocales($html = true) {
	...
	write_log("Locale language should be $lang", "INFO");
	if (is_dir($dir)) {
		if ($dh = opendir($dir)) {
...
function multiCurl($urls, $timeout = 10) {
	...
	foreach ($res as $url => $response) {
		...
		try {
			...
			write_log("An exception occurred: $e", "ERROR");
		}
...
function protectMessage($string) {
	...
	if (($_SESSION['cleanLogs'] ?? true) && !isWebApp()) {
		...
		foreach ($matches as $match) if (isset($match[0])) {
			...
			if (is_array($decoded)) {
				$keys = [
					'X-Plex-Token', 'apiToken', 'plexToken', 'authToken', 'token', 'email', 'username',
