...
class gitUpdate
{
    ...
    public function __construct()
    {
		...
		$repository = false;
    	if (file_exists("$paths/.git")) {
    		$repository = "$paths/.git";
	    } else {
    		$path = explode("/", $paths);
    		while (count($path) && !$repository) {
    			array_pop($path);
    			$test = implode("/", $path) . "/.git";
    			if (file_exists($test)) $repository = $test;
		    }
	    }
		if (!$repository) return false;
		write_log("Repository: $repository", "INFO", false, true);
        exec("git", $lines);
